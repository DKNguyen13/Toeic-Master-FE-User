...
const HistoryPage: React.FC<HistoryProps> = ({
  ...
  const handleStartTest = () => {
    // Điều hướng đến trang làm bài test
    window.location.href = "/tests"; // Hoặc dùng navigate từ react-router-dom
  };

  if (loading) return <p className="text-center mt-10">Loading...</p>;
  if (error) return <p className="text-center text-red-500">{error}</p>;

  // Kiểm tra nếu không có session nào
    ...
    <div className="min-h-screen flex flex-col">
            ...
            <div className="flex-1 flex flex-col items-center justify-center border-r border-gray-200 pr-6">
              ...
              <div className="flex space-x-6 mt-4 text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    Listening: {statistics?.averageListeningScore || 0}
                  </span>
                </div>
                <div className="flex items-center gap-2">
      ...
      <div className="relative py-6">
        ...
        <div className="relative flex justify-center">
          <span className="bg-gray-50 px-6 py-2 text-sm font-semibold text-gray-600 rounded-full shadow-sm">
            Lịch sử làm bài
          </span>
        </div>
      </div>
      {/* Phần dưới: Danh sách bài test hoặc Empty State */}
      <div className="bg-white p-8">
        {hasNoSessions ? (
          /* Empty State - Hiển thị khi chưa có bài thi */
          <div className="flex flex-col items-center justify-center py-16 px-4">
            ...
            <p className="text-gray-600 text-center max-w-md mb-8">
              Bạn chưa thực hiện bài thi nào. Hãy bắt đầu làm bài để xem kết quả
              và theo dõi tiến độ học tập của bạn nhé!
            </p>

            <Link to={`/tests`}>
              <button
                onClick={handleStartTest}
                className="flex items-center gap-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-semibold py-3 px-8 rounded-lg shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200"
              >
            ...
            <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6 max-w-3xl">
              <div className="text-center p-4">
                ...
                <p className="text-sm text-gray-600">Chọn bài test phù hợp</p>
              </div>
              <div className="text-center p-4">
                <div className="bg-orange-100 rounded-full w-12 h-12 flex items-center justify-center mx-auto mb-3">
                  <span className="text-orange-600 font-bold text-lg">2</span>
                </div>
                <p className="text-sm text-gray-600">Hoàn thành bài thi</p>
              </div>
              <div className="text-center p-4">
                <div className="bg-green-100 rounded-full w-12 h-12 flex items-center justify-center mx-auto mb-3">
                  <span className="text-green-600 font-bold text-lg">3</span>
                </div>
                <p className="text-sm text-gray-600">Xem kết quả chi tiết</p>
              </div>
            </div>
          </div>
        ) : (
          /* Danh sách bài test - Hiển thị khi có dữ liệu */
